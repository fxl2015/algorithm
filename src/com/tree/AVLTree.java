     ...
     3
      \
       4
        \
         5
显而易见——这棵二叉搜索树其实等同于一个链表了，也就是说，它在查找上的优势已经全无了——在这种情况下，查找一个结点的时间复杂度是O(N)！
...
public class AVLTree<T extends Comparable<T>> {
    ...
    private AVLTreeNode<T> insert(AVLTreeNode<T> tree, T key) {
        if (tree == null) {
            // 新建节点
            tree = new AVLTreeNode<T>(key, 0, null, null);
            if (tree==null) {
...
class AVLTreeNode<T extends Comparable<T>>{
	...
	public AVLTreeNode(T key,int height,AVLTreeNode left,AVLTreeNode right){
		this.key=key;
		this.height=height;
		this.left=left;
		this.right=right;
